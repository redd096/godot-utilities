...
func _ready() -> void:
	...
	# set buttons
	cancel_button.pressed.connect(func() -> void: if select_device_manager.is_initialized: select_device_manager.press_cancel())
	confirm_button.pressed.connect(func() -> void: if select_device_manager.is_initialized: select_device_manager.press_confirm())
	# update label
	var line1: String = str("Move device with ", select_device_manager.move_right, " and ", select_device_manager.move_left)
	var line2: String = str("\nPress ", select_device_manager.confirm, " to confirm or ", select_device_manager.cancel, " to cancel")
	label_in_scene.text = line1 + line2
	# destroy placeholders (be sure to not remove instantiated columns)
	for element: Node in columns_container.get_children():
...
func recreate_prefabs(devices_positions: Dictionary[int, int], show_unused_column: SelectDeviceManager.UnusedColumnPosition) -> void:
	...
	for element: ExampleDeviceElementUI in device_elements:
		element.queue_free()
	device_elements.clear()
	for empty: Node in empty_elements:
		empty.queue_free()
	...
	for device: int in devices_positions:
...
func instantiate_element(device: int, is_keyboard: bool, column_index: int) -> void:
	...
	if use_empty_to_fill_row:
		for i: int in range(0, columns.size()):
