...
func _ready() -> void:
	...
	# update label
	var line1: String = str("Press ", select_device.add_player, " to connect device")
...
func add_player_device(player_index: int, device: int, is_keyboard: bool) -> void:
...
func remove_player_device(player_index: int, _device: int, _is_keyboard: bool) -> void:
	if select_device.on_disconnect_resize_array:
		...
		for i: int in range(player_index, instantiated_labels.size()):
