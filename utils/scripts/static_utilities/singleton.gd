...
static var _instances: Dictionary[String, Variant] = {}
...
static func check_instance(obj: Node, set_dont_destroy_on_load: bool = true, destroy_copies: bool = true) -> bool:
	...
	var script_type: Script = obj.get_script()
...
static func instance(script_type: Object, auto_instantiate: bool = false) -> Variant:
	...
	var type: String = _get_string_from_script_type(script_type)
	if _instances.has(type):#Engine.has_singleton(type):
		var current_instance: Variant = _instances[type]#Engine.get_singleton(type)
		if current_instance:
			return current_instance
	# if there isn't an instance ot it's not valid, try find in scene
	var obj_instance: Variant = _find_object_of_type(script_type)
	# or instantiate it
...
static func _find_first_children_component_recursive(node: Node, script_type: Object) -> Variant:
	for child: Node in node.get_children():
		...
		var child_component: Variant = _find_first_children_component_recursive(child, script_type)
		if child_component:
