...
func _add_gamepad_button(action: StringName, joypad_button: JoyButton) -> bool:
	...
	if not ProjectSettings.has_setting(action_setting_path):
		_warning_message(str("There is NOT '", action, "' in InputMap. I'm creating it with InputEvent: ", joypad_button))
	...
	if _contains_joypad_button(events, joypad_button):
		_warning_message(str("Action '", action, "' already contains InputEvent: ", joypad_button))
		return false
...
func _set_canvas_internal(stretch_mode: String, success_message: String) -> void:
	...
	# if not already set
		...
		_warning_message(str("Canvas stretch mode is already '", stretch_mode, "'"))
