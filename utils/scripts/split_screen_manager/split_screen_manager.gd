...
func update_split_screen() -> void:
	var players_ok: bool = manage_exceed_players()
	var cameras_ok: bool = get_cameras()
	var rects_ok: bool = get_viewports_rects()
...
func manage_exceed_players() -> bool:
	...
	for i: int in range(number_of_players, players.size()):
		...
		if when_exceed_players == EManageExceedPlayers.DESTROY:
			...
			if players[i] is Node2D:
				(players[i] as Node2D).hide()
			elif players[i] is Node3D:
				(players[i] as Node3D).hide()
			else:
				push_warning("Player ", i, "is NOT a Node2D or Node3D")
