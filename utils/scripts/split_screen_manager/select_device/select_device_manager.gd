...
@export var can_navigate_with_inputs: bool = true
@export var move_left: String = "ui_left"
@export var move_right: String = "ui_right"
@export var can_confirm_with_inputs: bool = true
@export var confirm: String = "ui_accept"
@export var can_cancel_with_inputs: bool = true
...
func on_changed_devices_connection() -> void:
	...
	for joypad: int in connection_devices.connected_joypads:
...
func get_players_devices() -> Array[Array]:
	...
	for device: int in devices_positions:
		var player_index: int = devices_positions[device]
...
func move_device(device: int, move_device_to_right: bool) -> void:
	if move_device_to_right:
		var max_pos: int = maxi(number_of_players - 1, get_unused_column_index())
		var new_position: int = mini(devices_positions[device] + 1, max_pos)
		devices_positions[device] = new_position
	else:
		var min_pos: int = min(0, get_unused_column_index())
		var new_position: int = maxi(devices_positions[device] - 1, min_pos)
		devices_positions[device] = new_position
	# call event
	on_update_devices_positions.emit(devices_positions, show_unused_column)
